...
public interface NoticeRepository extends JpaRepository<NoticeEntity, Integer> {
    ...
    List<NoticeEntity> findByUseYn(String useYn);

    // 3. 페이징 처리가 포함된 전체 조회 (관리자 페이지용)
    Page<NoticeEntity> findAll(Pageable pageable);

    // 4. 특정 날짜 사이에 전시되는 공지사항 조회 (예시)
    List<NoticeEntity> findByDispStartDateLessThanEqualAndDispEndDateGreaterThanEqual(String now1, String now2);

    @Modifying // 데이터 변경이 일어나는 쿼리임을 명시
    @Query("UPDATE NoticeEntity n SET n.readCount = n.readCount + 1 WHERE n.noticeId = :id")
