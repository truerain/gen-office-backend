...
public class NoticeController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Void>> save(@RequestBody @Valid NoticeRequest request) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<NoticeResponse.ListDto>>> list() {
        return ResponseEntity.ok(ApiResponse.ok(noticeService.findAll()));
    }

    @Operation(summary = "공지사항 단건 상세 조회")
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<NoticeResponse.DetailDto>> detail(@PathVariable Integer id) {
        return ResponseEntity.ok(ApiResponse.ok(noticeService.findDetailById(id)));
    }

    @Operation(summary = "공지사항 조회수 증가")
    @PatchMapping("/{id}/read-count")
    public ResponseEntity<ApiResponse<Void>> incrementReadCount(@PathVariable Integer id) {
    ...
    @PostMapping("/bulk")
    public ResponseEntity<ApiResponse<Void>> bulkCommit(@RequestBody BulkNoticeRequest request) {
